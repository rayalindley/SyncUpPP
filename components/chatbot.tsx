...
export default function Chatbot() {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await axios.post("/api/dialogflow", { query: userMessage });
      ...
      {open && (
        <div className="w-80 h-96 bg-white rounded-lg shadow-lg flex flex-col overflow-hidden">
          <div className="pt-2 pb-2 pl-4 pr-4 bg-primary rounded-t-lg flex justify-between items-center text-white">
            <div>
              <h2 className="text-md font-bold">BotBot</h2>
              <h5 className="text-xs"> Not a lie, just smart replies! </h5>
            </div>
          ...
          <div className="flex-1 overflow-auto p-2 space-y-2 text-sm">
            {messages.map((msg, idx) => (
              ...
              >
                <div
                  className={`px-3 py-1 rounded-md max-w-[75%] ${
                    msg.sender === "user"
          ...
          <div className="p-2 pt-4 mb-2 border-t flex gap-2">
            <input
              type="text"
              value={input}
              ...
              className="flex-1 border border-primary rounded px-2 py-1 text-sm text-black"
            />
            <button
              onClick={handleSend}
              className={`px-3 py-1 rounded text-sm text-white ${
                isLoading ? "bg-charleston" : "bg-primary"
              }`}
              disabled={isLoading}
            >
