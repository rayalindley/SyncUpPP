...
export default function TagsInput({
  ...
  return (
    <div className={`w-full ${className}`} ref={containerRef}>
      <div
        className={`flex flex-wrap gap-1 rounded-md border-0 bg-white/5 p-1 shadow-sm ring-1 ring-inset ring-white/10 focus-within:ring-2 focus-within:ring-inset focus-within:ring-primary ${
          disabled ? "opacity-50" : ""
        }`}
      >
        {tags.map((tag) => (
          <span
            key={tag}
            className={`text-primary-foreground inline-flex items-center rounded-md bg-primary px-1 py-0.5 text-xs font-medium ${tagClassName}`}
          >
            {tag}
            <button
              type="button"
      ...
      {isFocused && filteredSuggestions.length > 0 && tags.length < maxItems && (
        <div className="relative">
          <ul
            className={`absolute z-10 mt-1 max-h-40 w-full overflow-auto rounded-md border border-[#525252] bg-charleston shadow-sm ${dropdownClassName}`}
          >
            {filteredSuggestions.map((suggestion) => (
              <li
                key={suggestion}
                className="cursor-pointer px-3 py-1 text-white hover:bg-primary/10"
                onMouseDown={(e) => {
                  e.preventDefault();
